...
# with what ``eosquality index`` emits (see vectorindex/backend.py::build).
...
def ensure_library_downloaded(
    ...
    _console.rule(
        f"[bold]Downloading reference library[/bold] [cyan]{dirname}[/cyan]"
    )
    ...
    with tempfile.TemporaryDirectory(
        prefix=f".{dirname}.", dir=cache_dir
    ) as tmp_str:
